...
import {metricAlarmOptionsToString, parseStatisticOption} from './alarm-config.mjs';
...
describe('parseStatisticOption', () => {
    ...
    console.log('Imported metricAlarmOptionsToString:', typeof metricAlarmOptionsToString);
    console.log('Imported parseStatisticOption:', typeof parseStatisticOption);
    console.log('Is parseStatisticOption undefined?', parseStatisticOption === undefined);
    console.log('Full import content:', {metricAlarmOptionsToString, parseStatisticOption});

// If it's an object with properties, inspect it
    if (typeof parseStatisticOption === 'object' && parseStatisticOption !== null) {
        console.log('Properties of parseStatisticOption:', Object.keys(parseStatisticOption));
    }
    ...
    test('handles percentile statistics correctly', () => {
        const percentiles = ['p10', 'p50', 'p90', 'p95', 'p99', 'p99.9'];

        percentiles.forEach(percentile => {
            expect(parseStatisticOption(percentile, 'Average')).toBe(percentile);

            // Test with different casing
            expect(parseStatisticOption(percentile.toUpperCase(), 'Average')).toBe(percentile);
    ...
    test('returns default value for invalid statistics', () => {
        const invalidValues = [
            ...
            'TC(200%:300%)',         // Percentage over 100%
            ...
            'wm (10:90)',            // Space not allowed in abbreviated format
            'tm99%',                 // Wrong format for single value
            'TM90',                  // Uppercase abbreviated format is invalid
    ...
    test('handles edge cases correctly', () => {
        ...
        expect(parseStatisticOption('TM(10.5%:90.5%)', 'Average')).toBe('TM(10.5%:90.5%)');
