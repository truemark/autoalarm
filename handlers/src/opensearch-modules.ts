...
const defaultThresholds: {
  ...
  [AlarmClassification.Critical]: {
    ClusterStatus_red: 1,
    ClusterStatus_yellow: 1,
    FreeStorageSpace: 5000,
...
const metricConfigs = [
  {metricName: 'ClusterStatus.red', namespace: 'AWS/ES'},
  {metricName: 'ClusterStatus.yellow', namespace: 'AWS/ES'},
  {metricName: 'FreeStorageSpace', namespace: 'AWS/ES'},
  {metricName: 'JVMMemoryPressure', namespace: 'AWS/ES'},
  {metricName: 'CPUUtilization', namespace: 'AWS/ES'},
];
...
export async function manageOpenSearchAlarms(
  ...
  for (const config of metricConfigs) {
    ...
    const {alarmName, thresholdKey, durationTimeKey, durationPeriodsKey} =
      await getAlarmConfig(domainName, type, metricName);
...
export async function deleteCWAlarm(
  ...
  if (alarmExists) {
    ...
    log
      ...
      .str('alarmName', alarmName)
      .str('domainName', domainName)
      .msg('Alarm does not exist for domain');
  }
}
